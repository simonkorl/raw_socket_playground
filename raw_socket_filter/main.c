        ...
        if (*iphead==0x45) { /* Double check for IPv4
            ...
            if(src_port == 5566 && dst_port == 5567) {
                ...
                fprintf(stdout,"Source host %d.%d.%d.%d\n",
                    iphead[12],iphead[13],
                    iphead[14],iphead[15]);
                fprintf(stdout,"Dest host %d.%d.%d.%d\n",
                    iphead[16],iphead[17],
                    iphead[18],iphead[19]);
...
void write_cb_func(evutil_socket_t fd, short what, void *arg) {
    ...
    send_ether("eno1", "01:02:03:04:05:06", ETH_P_IP, ip_data, ipv4_len, data->fd);
...
int main() {
    ...
    // bind interface to interface 
    if(BindRawSocketToInterface("eno1", socket, ETH_P_ALL)) {
    ...
    printf("bind finish\n");
    uint8_t mac[12] = {0};
    fetch_iface_mac("eno1", mac, socket);
    //! set up filter
    set_sock_not_src_mac_filter(socket, mac);
